...
async function checkImages(url) {
    ...
    // Wait for React to fully load the page
    await page.waitForSelector('img, svg, [src]', { timeout: 100000 });
    ...
    for (const image of images) {
        try {
            ...
            if (response.status >= 400) {
                ...
                // Append each valid image with its load time
                await appendToLoadTimeExcel([{ src: image.src, loadTime, xpath: image.xpath }], 'image_load_times.xlsx');
            }
        } catch (error) {
...
async function appendToLoadTimeExcel(data, filePath) {
    try {
        let workbook;
        if (fs.existsSync(filePath)) {
            workbook = xlsx.readFile(filePath);
        ...
        if (!worksheet) {
            worksheet = xlsx.utils.aoa_to_sheet([['Source Link', 'Load Time (ms)', 'XPath']]);
            xlsx.utils.book_append_sheet(workbook, worksheet, sheetName);
        }

        data.forEach(item => {
            const newRow = [item.src, item.loadTime, item.xpath];
            const rowNum = xlsx.utils.sheet_add_aoa(worksheet, [newRow], { origin: -1 }).s;

            // Apply red color to the load time cell if it exceeds the threshold
            if (item.loadTime > loadTimeThreshold) {
                const cell = worksheet[`B${rowNum}`];
                if (!cell.s) {
                    cell.s = {};
                }
                cell.s.fill = { fgColor: { rgb: "FF0000" } }; // Red fill color
            }
        });

        xlsx.writeFile(workbook, filePath);
        console.log(`Load time data appended to ${filePath}`);
    } catch (error) {
        console.log(`Error appending load times to Excel ${filePath}:`, error.message);
    }
}
...
// Example usage
checkImages('https://www.bajajfinserv.in/').then(() => {
