...
(async () => {
    ...
    // Find all elements with class '.bi'
    const iconElements = await page.$$('.bi');

    if (iconElements.length === 0) {
        console.log('No icons with the class .bi found.');
    } else {
        totalIcons = iconElements.length;
        console.log(`Found ${iconElements.length} icons. Measuring load times and taking screenshots...`);
        ...
        for (let i = 0; i < iconElements.length; i++) {
            ...
            if (boundingBox && boundingBox.width > 0 && boundingBox.height > 0) {
                const screenshotPath = path.join(savePath, `icon-${i + 1}.png`);
                ...
                console.log(`Skipping icon ${i + 1}: Element has no width or height.`);
    ...
    // Write valid icons to Excel
    if (validIcons.length > 0) {
        const wb = XLSX.utils.book_new();
        const wsData = [['Index', 'XPath', 'Load Time (ms)']];

        // Append valid icons to worksheet data
    ...
    // Write slow-loading icons to Excel
    ...
    // Send summary alert to Microsoft Teams
    let summaryTable = `
        <table border="1">
            <thead>
                <tr>
    ...
    console.log('Summary alert sent to Microsoft Teams.');
})();
